...
from google_friendly_model.build_model import mpf_model, SequentialCrossSpectralSpd_matricesLayer
...
if __name__ == "__main__":
    ...
    # Create model
    snc_window_size_hp = 1377#1394  # 1754 #trial.suggest_int("snc_window_size", 800, 1800, step=18)  #162 1044#128#648# 1458 = 27*18*3
    ...
    if use_pretrained_model:
        pretrained_model = fit_and_take_the_best(model, train_ds, train_ds, log_dir, #number=trial.number,
                                                 model_name='model_best', epochs=55,#45,#35,#25,  # 30, 15
                                                 save_checkp=False)
    else:
        pretrained_model =  model#fit_and_take_the_best(model, train_ds, train_ds, log_dir, #number=trial.number,
                                                 # model_name='model_best', epochs=1,  # 30, 15
                                                 # save_checkp=False)

    # Before saving, check the model architecture
    print("Model being saved has these layer output shapes:")
    ...
    # initial_model_path = '/home/wld-algo-6/Production/WeightEstimation2K/google_friendly_model/logs/04-06-2025-16-23-36/initial_pre_trained_model.keras'
    ...
    for i in range (50):
        ...
        for person in persons_for_test:
            ...
            # take a model
            print(f"Attempting to load model from: {initial_model_path}")
            # custom_objects = {
            ...
            out_callback_test = OutputPlotCallback(person_dict, log_dir,
                                                   ...
                                                   picture_name=person + 'test'+str(i), data_mode='Test',
                                                   phase='Train')
            out_callback_train = OutputPlotCallback(person_dict, log_dir,
                                                    samples_per_label_per_person=10, used_persons=[person],
                                                    picture_name=person + 'train'+str(i), data_mode='Train',
                                                    phase='Train')

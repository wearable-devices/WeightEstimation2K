...
def objective(trial):
    ...
    # dicts_for_initial_train = {person: get_weight_file_from_dir(persons_dirs[person]['train'])[person] for person in
    #                            persons_for_train_initial_model}
    # dicts_for_initial_test = {person: get_weight_file_from_dir(persons_dirs[person]['test'])[person] for person in
    #                           persons_for_test}
    #
    # dicts_for_plot = {person: get_weight_file_from_dir(persons_dirs[person]['test'])[person] for person in
    #                   persons_for_plot}

    # unite_dicts_for_initial_train = dicts_for_initial_train#{k: v for d in dicts_for_initial_train for k, v in d.items()}
    # unite_dicts_for_initial_test =dict_for_initial_test# {k: v for d in dict_for_initial_test for k, v in d.items()}
    train_ds = MultiInputGenerator(person_dict, window_size=snc_window_size_hp, batch_size=batch_size_np, phase='all',
                                   labels_to_balance=labels_to_balance, epoch_len=epoch_len, person_names=persons_for_train_initial_model)
    val_ds = train_ds #MultiInputGenerator(dicts_for_initial_test, window_size=snc_window_size_hp, batch_size=batch_size_np,
    ...
    for person in persons_for_test:
        # if person in persons_for_test:  #
        #     train_dict = get_weight_file_from_dir(drs['train'])
        #     test_dict = get_weight_file_from_dir(drs['test'])
        ...
        if use_pretrained_model:
            ...
            model = create_attention_weight_estimation_model(**attention_snc_model_parameters_dict)
        #     # model = create_one_sensor_attention_weight_estimation_model(**attention_snc_model_parameters_dict)
        # model = create_conv_weight_estimation_model()
        # model.summary()
        total_params, trainable_params, non_trainable_params = count_parameters(model)

        train_ds = MultiInputGenerator(train_dict, window_size=snc_window_size_hp, batch_size=batch_size_np,
                                       labels_to_balance=labels_to_balance, epoch_len=epoch_len)
        val_ds = MultiInputGenerator(test_dict, window_size=snc_window_size_hp, batch_size=batch_size_np,
                                     labels_to_balance=labels_to_balance)

        x = train_ds.__len__()
        train_ds.__getitem__(0)

        out_callback = OutputPlotCallback(test_dict, trial_dir,
        ...
        callbacks = [
            ...
            FeatureSpacePlotCallback(test_dict, trial_dir, layer_name='tf.math.reduce_mean', proj='pca',
                                     metric="euclidean", picture_name=person + 'test_dict',
    ...
    return mean_val_mse  # max_val_loss
...
if __name__ == "__main__":
    persons_for_train_initial_model = ['Avihoo', 'Aviner']
    persons_for_test = [ 'Leeor','Leeor1','Leeor2'
                        'Liav', 'Daniel', 'Foad'
                        ]

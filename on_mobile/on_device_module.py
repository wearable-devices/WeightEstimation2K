...
class OnDeviceModel(tf.Module):
  ...
  def __init__(self):
    ...
    self.model.compile(#losses=[None, tf.keras.losses.MeanSquaredError(), tf.keras.losses.MeanSquaredError(),
                  ...
                  loss = {'out': None, 'out1': tf.keras.losses.MeanSquaredError(), 'out2': tf.keras.losses.MeanSquaredError(),
                          'out3': tf.keras.losses.MeanSquaredError()},
                  metrics=['mae'],
                  optimizer=opt)
    # self.model.compile(
    #     optimizer='sgd',
  ...
  def train(self, snc1, snc2, snc3, y):
    ...
    with tf.GradientTape() as tape:
      ...
      # loss = self.model.loss[0](y, predictions)
